$(async function() {
  ...
  function generateFavs() {
    ...
    // loop through all of our favs and generate HTML for them
    for (let story of favs) {
      const result = generatefavHTML(story);
  ...
  function generatefavHTML(story) {
    ...
    const storyMarkup = $(`
      <li class="${story.storyId}">
        ...
        <small class="article-hostname ${hostName}">(${hostName})</small>
        <small class="article-username">posted by ${story.username}</small>
        <button class ="unfav">Un-Favorite</button>
      </li>
    `);
  ...
  const storySubmit = document.querySelector('#s');
  storySubmit.addEventListener('click', async function(e){
    e.preventDefault();
  ...
  const remove = document.querySelector('#all-articles-list');
  ...
  const fav = document.querySelector('#all-articles-list');
  fav.addEventListener('click', async function(e){
    if (e.target.className === 'fav'){
        const storyId = e.target.parentElement.id;
        const storyObj = await storyList.favorite(currentUser, storyId);
        
        let hostName = getHostName(storyObj.url);
        const newli = $(`
          <li class="${storyObj.storyId}">
          <a class="article-link" href="${storyObj.url}" target="a_blank">
            <strong>${storyObj.title}</strong>
          </a>
          <small class="article-author">by ${storyObj.author}</small>
        ...
        `);
        $favoritedArticles.append(newli);
    }
  })

  const unfav = document.querySelector('#favorited-articles');
  unfav.addEventListener('click', async function(e){
    if (e.target.className === 'unfav'){
      const storyId = e.target.parentElement.className;
      e.target.parentElement.remove();
